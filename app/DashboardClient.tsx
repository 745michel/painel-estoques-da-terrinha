...
const PRODUTOS_DESCONTINUADOS_MANUALMENTE = new Set([
  ...
  // Marcados sem giro a pedido do usuario em 03/09/2026:
  "MP - OLEO DE SOJA 900 ML CX  20",
  "MP - FARINHA DE MANDIOCA CRUA GROSSA KG",
  "MP - FUBA MIMOSO S/ FERRO KG",
]);
  ...
  return <main className="app-shell">
    ...
    {selected && <div className="drawer-backdrop" onClick={() => setSelected(null)}>
      <div className="drawer" onClick={(event) => event.stopPropagation()}>
        ...
        {hasComparacao && <div className="value-source-note"><span style={{ marginRight: 16 }}><small>REVISÃO ATUAL </small><strong>{fullDate.format(localDate(escadinhaData.dataPublicacao as string))}</strong></span><span><small>REVISÃO ANTERIOR </small><strong>{fullDate.format(localDate(escadinhaData.dataPublicacaoAnterior as string))}</strong></span></div>}
        <div className="table-wrap">
          <table className="consumption-table escadinha-drawer-table">
            <thead><tr><th>Mês</th><th>Plano{hasComparacao ? ` (${deliveryColumnDate.format(localDate(escadinhaData.dataPublicacaoAnterior as string))} → ${deliveryColumnDate.format(localDate(escadinhaData.dataPublicacao as string))})` : ""}</th><th>Real</th><th>% de atingimento</th></tr></thead>
            <tbody>
...
function PedidosVendaDashboard({
  ...
  const agrupados = useMemo(() => {
    // estoque/saldo podem vir null por loja quando o BI nunca teve quantidade_estoque pra ela
    // (produto novo, sem historico) - soma so as lojas com dado real nesse caso. Quando o BI
    // falha mas ja tivemos um valor real antes, build_pedidos_venda.py usa esse ultimo valor
    // conhecido (cache local) em vez de null - marca a linha como estoqueDesatualizado, e aqui
    ...
    for (const p of filtered) {
      ...
      if (p.estoqueDesatualizado) {
        atual.estoqueDesatualizado = true;
        if (p.estoqueCapturadoEm && (!atual.estoqueDataMaisAntiga || p.estoqueCapturadoEm < atual.estoqueDataMaisAntiga)) {
          atual.estoqueDataMaisAntiga = p.estoqueCapturadoEm;
        }
      }
  ...
  const produtosComEstoqueDesatualizado = agrupados.filter((p) => p.estoqueDesatualizado).length;
      ...
      <div className="content consumption-content">
        ...
        <section className="kpi-grid" aria-label="Indicadores principais" style={{ gridTemplateColumns: "repeat(4, minmax(0,1fr))" }}>
          ...
          <div className="kpi-card healthy-card">
            ...
            <small>
              {produtosComEstoqueIncompleto > 0 && `${produtosComEstoqueIncompleto} produto(s) sem nenhum dado de estoque ainda. `}
              {produtosComEstoqueDesatualizado > 0 && `${produtosComEstoqueDesatualizado} produto(s) com estoque de leitura anterior (BI sem atualização agora).`}
              {produtosComEstoqueIncompleto === 0 && produtosComEstoqueDesatualizado === 0 && "Soma das unidades filtradas"}
            </small>
          </div>
        ...
        <section className="inventory-panel consumption-panel">
          ...
          <div className="table-wrap consumption-table-wrap"><table className="consumption-table pedidos-venda-table" style={{ tableLayout: "fixed", width: "100%", minWidth: 0 }}><thead><tr>
            ...
            {agrupados.map((p) => <tr key={p.cod} onClick={() => setSelected(p)} style={{ cursor: "pointer" }}>
              <td><div className="product-cell"><div><strong title={p.produto}>{p.produto}</strong><small>Cód. {p.cod} · {p.loja}</small><small>{p.categoria ?? "—"}{p.estoqueIncompleto && <span title="BI nunca teve dado de estoque pra pelo menos uma loja desse produto"> · sem estoque cadastrado</span>}{!p.estoqueIncompleto && p.estoqueDesatualizado && <span title={`BI sem atualização agora - usando a última leitura conhecida${p.estoqueCapturadoEm ? ` (${new Date(p.estoqueCapturadoEm).toLocaleString("pt-BR", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" })})` : ""}`}> · estoque desatualizado</span>}</small></div></div></td>
...
// Cores do proprio painel (laranja da sidebar + verde principal), nao mais roxo/azul
// genericos - pedido do usuario em 01/09/2026: "coloque uma cor que seja do estilo do
// painel em todos os graficos".
const FORN_ANO_CORES = ["#9c480d", "#176b45", "#2f8a61", "#c98218"];
...
function FornecedoresDashboard({
  ...
  // anoAtivo pode ser "todos" (sentinela, nao existe em anoData) - sem esse caso especial aqui,
  // o fallback pra anoMaisRecente sempre vencia e o botao "Todos" nunca fazia efeito nenhum
  // (achado real em 01/09/2026).
  const anoRanking = anoAtivo === "todos" || anos.includes(anoAtivo) ? anoAtivo : anoMaisRecente;
  const [query, setQuery] = useState("");
  ...
  const rankingContexto = useMemo(() => {
    let base = anoRanking === "todos" || !grupoAtual.anoData[anoRanking] ? combinadoTop : grupoAtual.anoData[anoRanking].top;
    ...
    const totalCaixas = base.reduce((s, r) => s + r.caixas, 0);
    return { lista: base, total, totalBruto, totalKg, totalCaixas, fornecedores: base.length };
  ...
  function produtosPorAno(getLista: (ano: string) => FornecedorProduto[]) {
    ...
    const linhas = [...nomes].map((nome) => {
      ...
      for (const ano of anos) {
        ...
        dados[ano] = item ?? { p: nome, valor: 0, valorBruto: 0, kg: 0, caixas: 0 };
  ...
  function renderTabelaProdutos(linhas: ReturnType<typeof produtosPorAno>) {
    ...
    const filtradas = produtosSelecionados.length > 0 ? linhas.filter((l) => produtosSelecionados.includes(l.nome)) : linhas;
    return <>
      ...
      <div className="content values-content">
        ...
        {produtoFocoAtivo && kpisProduto ? (
          ...
          <section className="value-kpis" aria-label={`Indicadores de ${focoFornecedores.join(", ")}`}>
            ...
            {/* TESTE, 10/09/2026, pedido do usuario: "Preço médio" de volta, agora em bruto e
                usando a mesma logica de precoBruto() (cx quando o(s) fornecedor(es) vende(m)
                por caixa, kg quando vende(m) por peso) em vez de misturar valor liquido com kg
                de todo mundo junto (jeito antigo, removido antes). Usa os totais ja filtrados
                de rankingContexto (mesmos fornecedores focados). */}
            <div className="value-kpi"><span>Preço médio</span><strong>{precoBruto({ valorBruto: rankingContexto.totalBruto, kg: rankingContexto.totalKg, caixas: rankingContexto.totalCaixas }) || "—"}</strong><small>&nbsp;</small></div>
